...
const { WorkContentModel, WorkPublishContentModel } = modelsWorkContentModel;

/**
 * 创建作品
 */
async function createWorkService(
 ...
 * 更新作品数据
 */
async function updateWorkService(
	data: {
		content?: {
			components: [object];
			props: object;
 ...
 * 查询 作品/模板 列表
 */
async function findWorkListService(
	whereOpt: {
		id?: string;
		uuid?: string;
		author?: string;
	...
	// ------------ 拼接查询条件 ------------
	const wheres: { [propName: string]: unknown } = {};
	...
	const pageSizeNumber = pageSize; // 有可能传入进来是 string 类型的
	const pageIndexNumber = pageIndex;
